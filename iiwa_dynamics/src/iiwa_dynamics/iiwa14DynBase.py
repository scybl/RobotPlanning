...
class Iiwa14DynamicBase(Node):
    def __init__(self, tf_suffix=''):
        super().__init__(f'iiwa_dyn_{tf_suffix}' if tf_suffix else 'iiwa_dyn_base')
        ...
        # ROS related
        self.joint_state_sub = self.create_subscription(JointState, '/iiwa/joint_states',
                                                        self.joint_state_callback, 5)
        self.traj_publisher = self.create_publisher(JointTrajectory,
                                                    '/EffortJointInterface_trajectory_controller/command', 5)
